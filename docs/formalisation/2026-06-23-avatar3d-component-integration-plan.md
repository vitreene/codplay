...
|---|---|---|
| **Preload (par URL)** | `GLTFLoader.load(url)` → `gltf.scene` brut, non traversé, non retargeté, stocké dans le cache preload (`getEntry`/`setEntry`, comme `RivePreloadEntry`) | partagé entre toutes les instances référençant la même URL |
| **`init()` (par instance)** | `SkeletonUtils.clone(cachedScene)` (clone nécessaire — bones/morphs indépendants par instance) → traversal → strip `morphPrefix` → `retarget()` → `engine.registerBlendMorph(...)`, tous lus depuis `perso.initial` ; construction de `WebGLRenderer`/`Scene`/`Camera` propres à l'instance à partir du canvas créé en `render()` ; `scene.add(clonedGroup)` ; enregistrement dans le hub (closure `instances`, comme `create-rive-binding.ts`) | par instance |
...
La méthode change de contrat : elle ne fait plus le fetch (ça devient la stratégie preload), elle reçoit la scène GLTF déjà en cache et fait seulement clone + traversal + retarget + registration — c'est la partie qui reste réellement "par instance".
...
**Non vérifié visuellement** : je n'ai pas de navigateur disponible dans cet environnement (contrainte : ne jamais installer Chromium). Vérifié : typecheck (`avatar3d`, `avatar-engine` propres ; `codplay`/`demos` sans nouvelle catégorie d'erreur par rapport à la baseline), tests unitaires (`avatar3d`: 8/8, `avatar-engine`: 5/5, suite `codplay` complète : mêmes 13 échecs pré-existants), transformation Vite de tous les fichiers modifiés (200 via curl sur le dev server), et que `/avatars/avatarsdk.glb` est bien servi. Reste à confirmer en navigateur : le rendu visuel réel (avatar visible, idle animations, gestes, seek) sur `avatar-poc-1`.
