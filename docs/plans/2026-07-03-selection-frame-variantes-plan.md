...
**Phase 1 — mode création du cs**

1. **Extraction `gesture-session.ts`** — refactor sans changement de comportement ; les specs cs existantes restent vertes.
2. **Mode création** : branche `creating` de la machine du cs, outils de tracé libre/grid (`creation-tool.ts`), `applyCreationGeometry`, émission `onCreate`, `attachItem` → bascule en sélection régulière avec le même cadre.
3. **Démo `?demo=selection-frame-create`** : création par tracé dans la scène puis dans une capsule grid, avec l'enchaînement complet tracé du cs → création du perso → `attachItem` → sélection régulière. Les contrôles fonctionnels sont des persos layout+emit de la story, pas des boutons externes.
4. **Validation utilisateur** du mode création — point d'arrêt avant la phase 2.

**Phase 2 — éditeur de zones**

5. **`zone-model.ts`** — état + opérations pures : nommage auto, chevauchement, split avec faux gap (parts valides `(span − (n−1)×g) % n === 0`), merge (englobant), ajustement numérique rangées/colonnes pour les pistes réservées, interdiction de `gap` CSS au-delà du seuil. Entièrement testable sans DOM.
6. **`createZoneEditor` — affichage** : accrochage capsule, gabarit éditable (matrice complète + templates mesurés), rendu des zones, régimes normal / basse définition (macro-cellules à pas régulier, pistes réservées représentées, survol pleine résolution).
7. **`createZoneEditor` — gestes** : tracé, sélection/multi-sélection, poignées d'emprise, déplacement ; commandes programmatiques sur le même chemin de mutation (une card éditeur = `setState`, déjà couvert).
8. **Démo `?demo=zone-editor`** : capsule grid fine (ordre 160×90), tracé de zones, pose d'une card (titre/corps/footer), split avec faux gap, merge — contrôles layout+emit dans la story.
