...
`capsule-automation` est l'autorité du placement explicite `{ row, col, rowSpan, colSpan }`, des règles CSS et de leurs classes. Le builder ed2 actuel utilise déjà ce package pour les grilles et les placements automatiques, mais construit les `AutoCapsuleChildInput` sans `placement` et ne transmet pas `scene.zones`/`Decor.zoneId`.
