...
| 5 | Spec capsule (ed2) | Spec de conception | Écrite ; les décisions capsule principales sont consignées, mais la section zones reste provisoire et l'intégration n'est pas faite | `packages/editor/plan/2026-07-08-capsule-spec.md` |
| 6 | Spec modèle d'Item (ed2) | Spec | Écrit, fermée (tous les points tranchés) | `packages/editor/plan/2026-07-08-item-model-spec.md` |
| 7 | Éditeur de zones (selection-frame) | Spécification + plan d'intégration | Modèle, gestes et rendu unitaire vérifiés dans la [spec zone-editor](../../authoring/selection-frame/specs/zone-editor-spec.md) ; raccords hôte, drop, attache par `id` et validations navigateur restent au [plan d'intégration](../../authoring/selection-frame/plan/zone-editor-plan.md) | `packages/authoring/selection-frame/` |
...
| 16 | Reprise de l'éditeur avec CodPlay V2 | Plan de migration | **En cours (2026-08-30)** — tranche 2 (builder V2 minimal) autorisée ; `snapshot`, bridge et Decor restent à intégrer selon le rapport | `packages/editor/plan/2026-08-30-editor-codplay-v2-reprise-report.md` |
...
5. **Spec capsule (ed2)** — dépend de (1) et (3) : consigne les décisions de conception du comportement capsule, dont le mapping `CapsuleKind` → `GRID_MODE` et la fusion `position`/`card`. Sa section zones demeure provisoire ; les coordonnées, clés d'attache et placements attendent la réconciliation avec le modèle vérifié de selection-frame et le plan de migration dedit.
...
7. **Éditeur de zones** — le modèle et l'éditeur selection-frame existent et sont vérifiés par les tests du package. Leur raccord avec le modèle dedit, les attaches persistantes et le drop du cadre reste à exécuter selon `packages/authoring/selection-frame/plan/zone-editor-plan.md`.
8. **Migration `ZoneDef` dedit** — dépend de (7) : dedit conserve aujourd'hui des rectangles cqw et des références par nom, tandis que selection-frame produit des identifiants stables et des emprises en pistes. L'identité, le nommage et les surfaces par orientation restent à réconcilier avant le code, selon le [plan de migration](./modules/2026-07-08-dedit-zonedef-migration-plan.md).
