...
| — (pas dans le plan d'origine) | `scene.state` inaccessible | **Fait** — sur le modèle de `scene.initial.set`. D'abord parqué (aucun besoin ed2 identifié), puis l'utilisateur a tranché sans ambiguïté que `SceneDocEditor` doit le porter quel que soit l'usage ed2 — capacité de l'outil, pas conditionnée à un besoin immédiat. L'usage réel dans quiz-hunt (état partagé cross-stories) reste, lui, différé à la reprise de ce chantier. |
...
- **`scene.state`** : proposé initialement dans le plan d'audit uniquement parce que le type le porte, aucun besoin identifié à ce moment-là. L'utilisateur a d'abord évoqué un souvenir d'usage dans quiz-hunt pour un « résultat global » — vérifié dans le code, infirmé (quiz-hunt n'a que du `story.state`, pas de `scene.state`) ; l'utilisateur a alors accepté de différer l'ajout au retour sur quiz-hunt. **Revenu dessus dans la foulée, sans ambiguïté** : « SceneDocEditor doit avoir un scene.state » — décidé comme capacité de l'outil, indépendamment de tout usage ed2 immédiat. Ajouté, sur le modèle de `scene.initial.set`. L'usage réel dans quiz-hunt (état partagé cross-stories) reste différé, cf mémoire de session dédiée.

Implémenté et vérifié : `id?`/collision sur `createStory`/`createPerso`, `scene.onStart`/`scene.onSequenceEnd`, `scene.state`, `cloneStory()` préserve `trackId`, `Perso.list?: ListConfig` ajouté côté Codplay et suivi par `clonePerso()`. 7 nouveaux tests dans `packages/codplay/tests/v1/creator-api.spec.ts` (279 tests au total, tous verts). `docs/formalisation/v1-authoring-api.md` mis à jour en premier, l'implémentation ensuite — pas l'inverse.

**Complément (même jour) — `SceneDef.name`** : nouveau principe posé par l'utilisateur, `feedback-scene-story-parameter-symmetry` en mémoire de session — quand une propriété est ajoutée/modifiée à un niveau (scène ou story), se demander si l'autre niveau en a besoin aussi, sans viser une stricte équivalence. Appliqué en direct : `name` (déjà sur `StoryDef`/`Perso`) manquait sur `SceneDef`. Ma première lecture (le mécanisme id-dérivé-du-nom ne s'applique pas à la scène, dont l'id est toujours explicite) a été dépassée par l'utilisateur : facultatif, donc sans risque, à ajouter quand même. `SceneDef.name?: string` ajouté côté Codplay, `create({id, name?})` dans `SceneDocEditor`, aucun setter séparé (comme pour `story.name`/`perso.name`, fixé à la création).
