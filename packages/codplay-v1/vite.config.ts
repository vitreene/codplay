...
export default defineConfig({
  resolve: {
    alias: [
      ...
      // When demo files (loaded transitively via @codplay/demos) import back from codplay-v1, resolve to local src
      { find: /^codplay-v1\/(.*)/, replacement: resolve(__dirname, './src/$1') },
      { find: 'codplay-v1', replacement: resolve(__dirname, './src/index.ts') },
    ],
  },
})
