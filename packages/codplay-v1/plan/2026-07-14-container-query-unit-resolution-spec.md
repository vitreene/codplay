...
Quatre points d'appel réels, tous dans `packages/codplay-v1/src/` — les deux autres points listés dans une première version de cette spec (`create-flip-engine.ts::applyChannelState`, `create-list-flip-module.ts`) sont exclus : leurs valeurs sont typées `number` (deltas mesurés par FLIP), jamais des chaînes cq-unit, donc hors sujet.
