...
implémentés dans `packages/codplay-v1/src/player/player.ts:482-493` via `readNodePose`/
`readNodeSnapshot`, `packages/codplay-v1/src/runtime/components/lib/dom.ts`) avaient pour intention
...
`packages/codplay-v1/src/animation/adapter.ts` traite déjà `transition.target` de façon générique
...
Trouvé en lisant `packages/codplay-v1/src/animation/adapter.ts` (déjà exploré plus tôt dans la même
...
- `operations: AnimationOperation[]` = `transitionGroup.transitions` (ligne 543) — les
  ...
  (`packages/codplay-v1/src/animation/types.ts:69-92`) porte `listenerId` (le **persoId**, confirmé
...
Trouvé par lecture de code (`packages/codplay-v1/src/runtime/components/runtime-component-orchestrator.ts:442-449`,
