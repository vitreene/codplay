...
1. **Codplay fournit une capacité réutilisable et laisse l'appelant décider** quand un étage supérieur
   existe (horloge, catalogue de capacités, manifeste et moment du preload). La diffusion autonome dispose
   de la façade `run` comme enchaînement par défaut.
