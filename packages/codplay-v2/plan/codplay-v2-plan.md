...
| Events, listen et straps | Pipeline `listen -> transform -> straps -> emit -> persos`, fonctions referencees, ordre stable, events comme contrat primaire | Sous-tranche `listen -> transform -> emit` en place; straps asynchrones et materialisation des sorties restent a ouvrir. |
