...
| Events, listen et straps | Pipeline `listen -> transform -> straps -> emit -> persos`, fonctions referencees, ordre stable, events comme contrat primaire | Propagation, executeur sequentiel, collections, validation init et sorties event vers tracks dediees en place; helpers V2 et `update` restent a ouvrir. |
| Helpers de straps et schedule | Delais, repetitions, stagger, `planned` et cas `live` | Helpers planned finis en place; loop et tout contrat live restent a specifier avec `f(t)`. |
