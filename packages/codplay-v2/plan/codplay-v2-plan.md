...
| Mouvement HTML | Tranche HTML fixe et clôturée | FIRST/LAST exacts, modes local/reparent, profondeur arbitraire, circuit Play/Seek unique et optimisation sans lecture DOM par frame sont couverts ; le mouvement des éléments SVG suit le même circuit HTML/DOM lorsqu'un composant en produit. |
...
| Authoring éditeur | [`2026-07-12-app-controller-definition.md`](../../editor/plan/app/2026-07-12-app-controller-definition.md) et [`facade-engine-instance-plan.md`](./facade-engine-instance-plan.md) §8 | L'accès authoring V2 est externe au core : lecture par `getPersoStates()` et écriture d'un état temporaire `{ persoId, timeMs, state }` avant materialisation. `setDecor(decorId, patch)` reste le canal de persistance éditeur ; `setNodePose` et les autres accès au nœud sont optionnels et à vérifier dans la reprise de l'éditeur, sans devenir une API générique ni un circuit concurrent. |
