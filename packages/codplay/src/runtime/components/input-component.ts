...
export const INPUT_STANDARD_ACTIONS: Record<
  string,
  Pick<InputActionDoc, "disableAnswers" | "showCorrection">
> = {
...
export function resolveInputStandardActions(
  authoredActions: Record<string, InputActionDoc> = {},
 ...
 */
function resolveInputPartDefinition(input: unknown, fallback: InputPartDefinition = {}): InputPartDefinition {
...
function resolveInputState(
  ...
  const correctionIcon = resolveCorrectionIconDefinition(
    input.correctionIcon,
    fallback?.correctionIcon ?? {},
  );

  const resolvedBase: ResolvedInputState = {
    inputType:
      typeof input.inputType === "string" && input.inputType.length > 0 ?
        input.inputType
      : (fallback?.inputType ?? "text"),
    ...
    value: typeof input.value === "string" || typeof input.value === "number" ? input.value : fallback?.value,
    label:
  ...
  return {
    ...
    visualState:
      typeof input.visualState === "string" ? resolvedBase.visualState
      : resolvedBase.showCorrection ?
        isSelected && isCorrect ? "revealed-correct"
        : isSelected ? "revealed-incorrect"
        : isCorrect ? "revealed-missed-correct"
        : resolvedBase.disableAnswers ? "disabled"
        : "idle"
      : isSelected ? "selected"
      : resolvedBase.disableAnswers ? "disabled"
      : "idle",
...
export class InputComponent extends BaseComponent {
  ...
  private applyControlState(controlNode: unknown, state: ResolvedInputState): void {
    if (
      ...
      !(controlNode instanceof globalThis.HTMLInputElement)
    ) {
      return;
    }
  ...
  private applyState(
    ...
    const previousControlClasses =
      previousState !== null ? resolveInputControlStateClasses(previousState) : [];
    ...
    if (selectionIconNode !== null) {
      ...
      const selectionChildCount =
        (selectionIconNode as { childNodes?: { length?: number } }).childNodes?.length ?? 0;
      if (selectionChildCount === 0) {
        applyTextContent(
          selectionIconNode,
          resolveSelectedState(nextState) ? (nextState.selectionIcon.content ?? "") : "",
        );
      }
    }
    ...
    if (correctionIconNode !== null) {
      const previousCorrectionState =
        previousState === null ? "idle" : (resolveCorrectionState(previousState.visualState) ?? "idle");
      const nextCorrectionState = correctionState === null ? "idle" : correctionState;
      const previousCorrectionIconClasses =
        previousState !== null ?
          ["input__correction-icon", previousState.correctionIcon.className, `is-${previousCorrectionState}`]
            .filter((token) => typeof token === "string" && token.length > 0)
            .join(" ")
        : undefined;
      ...
      const correctionChildCount =
        (correctionIconNode as { childNodes?: { length?: number } }).childNodes?.length ?? 0;
