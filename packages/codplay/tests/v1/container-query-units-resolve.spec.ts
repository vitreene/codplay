...
import { resolveContainerQueryValue } from '../../src/runtime/components/lib/container-query-units'

/**
 * Builds a `.ac-scene-root` container with a stubbed rect, and one child
 * node inside it — jsdom never computes real layout, so every test that
 * needs a resolved dimension stubs `getBoundingClientRect` itself.
 */
...
describe('V1 - resolveContainerQueryValue', () => {
  ...
  it('leaves a container-query value untouched when no .ac-scene-root ancestor exists', () => {
