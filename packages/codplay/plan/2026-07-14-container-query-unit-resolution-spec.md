...
Le conteneur de requête est retrouvé depuis le node cible lui-même via `node.closest('.ac-scene-root')` — `ac-scene-root` est la seule classe du repo portant `container-type` (`packages/authoring/capsule-automation/src/core/build-grid.ts`), un identifiant fixe et non ambigu, pas une valeur à découvrir par parcours générique du DOM. Aucun paramètre nouveau à faire transiter par `player.init`/`createDefaultAnimationAdapter` : chaque point d'appel a déjà le node cible en main.

Si `closest` ne trouve rien (scène non authorée par ed2, ou perso hors de tout `ac-scene-root`), la valeur traverse sans résolution — ce module reste inerte pour un auteur qui n'utilise pas cette convention.
