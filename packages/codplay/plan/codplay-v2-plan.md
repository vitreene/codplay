...
| Replace | Module de remplacement, instantanés transitoires et exposition de contenu foreign | [`foreign-scene-component-plan.md`](./foreign-scene-component-plan.md) fixe la transposition V2 du module partagé : hooks génériques, surface de présentation HTML et chemin `replace-simple`/`fade` pour `slot` et `img`; `replace.split` y est ignoré pour `slot`. `tag`, les contenus asynchrones et l'intégration Sighty restent à valider. |
