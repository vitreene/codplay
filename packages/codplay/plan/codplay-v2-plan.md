...
| Authoring éditeur | [`2026-07-12-app-controller-definition.md`](../../editor/plan/app/2026-07-12-app-controller-definition.md), [`facade-engine-instance-plan.md`](./facade-engine-instance-plan.md) §8 et plan de reprise ed2 | L'accès authoring V2 reste externe au cœur métier : lecture par `snapshot`, écriture d'un état temporaire `{ storyId, persoId, timeMs, state }` avant materialisation, et géométrie numérique fournie par la projection HTML. `setDecor(decorId, patch)` reste le canal de persistance éditeur. `setNodePose`, `getNodePose` et les autres accès au nœud V1 sont abandonnés ; la géométrie de présentation est une obligation de la frontière `Projection.measure`, à exposer sans handle DOM ni circuit concurrent. |
