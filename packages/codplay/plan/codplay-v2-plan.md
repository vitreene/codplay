...
| Composants | Base générique et base HTML séparées, tranche HTML/DOM implémentée | `BaseComponent` est substrat-neutre et reçoit une facade de services abstraite ; `BaseHTMLComponent`, les huit composants core (`LayoutComponent`, `TagComponent`, `ListComponent`, `MediaComponent`, `ImageComponent`, `InputComponent`, `PolygonComponent`, `ForeignContentComponent`), leurs dossiers profils/validation/projection, classes runtime, parts/outlets et materialisation template string — y compris les fragments sans enveloppe et les éléments SVG — sont couverts par l'unique `RuntimeMaterializer` HTML/DOM ; JSX, Canvas et Three.js restent hors tranche. |
...
| Contenu foreign et hôte HTML | [`foreign-scene-component-plan.md`](./foreign-scene-component-plan.md) | En cours : profil `slot`, manifeste et surface HTML d'attachement engagés ; remplacement partagé, façade interinstances et intégration Sighty restent à construire |
...
| Replace | Module de remplacement, instantanés transitoires et exposition de contenu foreign | [`foreign-scene-component-plan.md`](./foreign-scene-component-plan.md) fixe la transposition V2 du module partagé : hooks génériques, surface de présentation HTML et chemin `replace-simple`/`fade` pour `slot` et `img`; `replace.split` y est ignoré pour `slot`. `tag`, les contenus asynchrones et les instances Sighty restent à valider. |
