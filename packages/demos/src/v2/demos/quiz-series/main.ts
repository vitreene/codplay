...
type QuizQuestionAnsweredPayload = QuizQuestionResolvedPayload
        ...
        initial: {
          ...
          markup: `
            <div class="quiz-series-wrapper">
              <div data-part="quiz-series:progress" class="quiz-series-progress-slot"></div>
              <div data-part="quiz-series:slot" class="quiz-series-slot"></div>
            </div>
          `,
          style: {
            width: "100%",
...
export function createSeriesProgressStory(options: SeriesProgressStoryOptions = {}): StoryDoc {
  ...
  const persos: PersoDoc[] = [
    ...
    },
    {
      ...
      initial: {
        ...
        move: { target: "quiz-series:progress-count" }
      },
  ...
  if (showTrack) {
    persos.push({
      ...
      initial: {
        ...
        move: { target: "quiz-series:progress-track" }
      },
  ...
  if (showRate) {
    persos.push({
      ...
      initial: {
        ...
        move: { target: "quiz-series:progress-rate" }
      },
...
function createSeriesQuestionStory(
    ...
    return [
      {
        ...
        initial: {
          ...
          disabled: false,
          visualState: "idle",
          selectionIcon: { className: `${prefix}-selection-icon-slot` },
          correctionIcon: {
            className: `${prefix}-correction-icon-slot`,
            correctContent: "+",
            incorrectContent: "-",
        ...
        actions: {
          [`${prefix}:answer:${answer.id}:selected`]: {
            selectedAnswerIds: [answer.id],
            checked: true,
            visualState: "selected"
          },
          [`${prefix}:answer:${answer.id}:idle`]: {
            selectedAnswerIds: [],
            checked: false,
            visualState: "idle"
          },
          [`${prefix}:answer:${answer.id}:revealed-correct`]: {
            selectedAnswerIds: [answer.id],
            correctAnswerIds: question.answers.filter((candidate) => candidate.isCorrect).map((candidate) => candidate.id),
            checked: true,
            disabled: true,
            disableAnswers: true,
            showCorrection: true,
      ...
      },
      {
        ...
        initial: {
          ...
          move: { target: selectionIconPartId(storyId, answerRootId) }
        },
        actions: {
      ...
      },
      {
        ...
        initial: {
          ...
          move: { target: correctionIconPartId(storyId, answerRootId) }
        },
        actions: {
  ...
  return {
    ...
    straps: {
      "quiz-series-question-select": ({ event, state }) =>
        handleSeriesQuestionSelect(prefix, state, event.data),
      "quiz-series-question-submit": ({ state }) =>
        handleSeriesQuestionSubmit(prefix, state)
    },
    listen: [
...
function createSeriesResultStory(): StoryDoc {
  const itemPersos: PersoDoc[] = Array.from({ length: SERIES_TOTAL }, (_, i) => {
    ...
    return {
      ...
      initial: {
        ...
        move: { target: "quiz-series-result:items" }
      },
      actions: {
    ...
    persos: [
      ...
      },
      {
        ...
        initial: {
          ...
          move: { target: "quiz-series-result:card" }
        },
        actions: {}
      },
      ...itemPersos,
      {
        ...
        initial: {
          ...
          move: { target: "quiz-series-result:score" }
        },
        actions: {
          "quiz:result:score": {}
        }
      },
      {
        ...
        initial: {
          ...
          move: { target: "quiz-series-result:verdict" }
        },
        actions: {
...
export function createQuizSeriesFameScene(): SceneDoc {
  ...
  const allStories: Record<string, StoryDoc> = {
    main: createSeriesContainerStory()
  }
...
function handleSeriesAggregate(
  ...
  const progressEvents: Array<{ name: string; data: CompiledRecord }> = [
...
/** Creates the public V2 scene module entry consumed by the common layout. */
export function createScene(): SceneDoc {
  return createQuizSeriesFameScene()
}
