...
function readChronoRange(data: Readonly<Record<string, unknown>> | undefined): Readonly<{
  ...
  const durationMs =
    typeof data?.duration === "number" && data.duration > 0 ? data.duration : Math.max(1, computedDurationMs);
...
// FIX le bouton "demarrer" n'apparait plus au seek back s'il est en position "arreter"
    ...
    stories: {
      main: {
        ...
        persos: [
          ...
          // FIX chrono-face devrait employer une composant svg  ou c'est sans interet ?
          {
            id: "chrono-face",
            type: "layout",
            initial: {
