...
export class PlayerFacade implements PlayerApi {
  ...
  async seek(targetTimelineMs: number): Promise<PlayerCommandResult> {
    ...
    if (this.status !== 'ready' && this.status !== 'paused' && this.status !== 'playing') {
      return this.reject('INVALID_PLAYER_STATE', 'seek is only allowed from ready, paused, or playing', 'player:seek', {
        currentState: this.status
      })
    }
  ...
  async rewind(): Promise<PlayerCommandResult> {
    ...
    if (this.status !== 'ready' && this.status !== 'paused' && this.status !== 'playing') {
      return this.reject('INVALID_PLAYER_STATE', 'rewind is only allowed from ready, paused, or playing', 'player:rewind', {
        currentState: this.status
      })
    }
  ...
  async rebuild(mode: RebuildMode = 'state'): Promise<PlayerCommandResult> {
    ...
    if (!this.runtimePolicy.allowedRebuildModes.includes(mode)) {
      return this.reject('MODE_NOT_ALLOWED_BY_POLICY', 'Requested rebuild mode is not allowed by policy', 'player:rebuild', {
        mode,
    ...
    if (mode === 'full') {
      ...
      const rendererLoadResult = this.renderer.load({ story: runtimePlan.story })
      if (!rendererLoadResult.ok) {
        return this.reject('RENDERER_LOAD_FAILED', 'Renderer failed to rebuild story', 'player:rebuild:failed', {
          sceneId: this.scene.id,
          mode,
          code: rendererLoadResult.error.code
        })
      ...
      if (previousStatus === 'playing' || previousStatus === 'paused') {
        ...
        if (!rendererStartResult.ok) {
          return this.reject('RENDERER_INVALID_STATE', 'Renderer could not resume after full rebuild', 'player:rebuild:failed', {
            sceneId: this.scene.id,
            mode,
        ...
        if (previousStatus === 'paused') {
          ...
          if (!rendererPauseResult.ok) {
            return this.reject('RENDERER_INVALID_STATE', 'Renderer could not restore paused state after full rebuild', 'player:rebuild:failed', {
              sceneId: this.scene.id,
              mode,
